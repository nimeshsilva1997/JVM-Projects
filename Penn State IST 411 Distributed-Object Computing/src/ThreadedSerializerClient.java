...
public class ThreadedSerializerClient {
	...
	public static void main(String[] args) {
		try {	
			...
			/* 
			 * Setup to read from the server with ObjectInputStream and write to the server with ObjectOutputStream
			 * 
			 * Since the ObjectOutputStream was already created in the server on the same address (IP and port) we need to
			 * "flush" it here in the client before we can use it
			 * 
			 * From the Java Doc - ObjectOutputStream.flush(): Flushes this output stream and forces any buffered output 
			 * bytes to be written out. The general contract of flush is that calling it is an indication that, if any 
			 * bytes previously written have been buffered by the implementation of the output stream, such bytes should 
			...
			//Continue to run the program and ask the user for details about animals until they indicate they are done (Enter "X")
			do {				
			...
			//When the user is done and wants to quit, close the client's connection to the server
			clientSocket.close();
